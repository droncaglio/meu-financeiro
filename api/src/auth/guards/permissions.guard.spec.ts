...
function mockContext(user: unknown, handler = jest.fn(), cls = jest.fn()): ExecutionContext {
  return {
...
describe('PermissionsGuard', () => {
  ...
  it('denies when no user in request', () => {
    jest.spyOn(reflector, 'getAllAndOverride').mockReturnValue(['accounts:read']);
    expect(guard.canActivate(mockContext(undefined))).toBe(false);
  });

  it('allows superUser regardless of permissions', () => {
    jest.spyOn(reflector, 'getAllAndOverride').mockReturnValue(['accounts:delete']);
  ...
  it('allows when user has all required permissions', () => {
    jest.spyOn(reflector, 'getAllAndOverride').mockReturnValue(['accounts:read', 'entries:read']);
    const user = { isSuperUser: false, permissions: ['accounts:read', 'entries:read', 'reports:read'] };
    expect(guard.canActivate(mockContext(user))).toBe(true);
  });

  it('denies when user is missing one required permission', () => {
    jest.spyOn(reflector, 'getAllAndOverride').mockReturnValue(['accounts:read', 'entries:write']);
